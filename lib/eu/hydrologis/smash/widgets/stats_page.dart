  ...
  Future<void> collectStats() async {
    ...
    if (hasNotes) {
      ...
      notesEnvWidth = CoordinateUtilities.getDistance(
          ...
          Coordinate.fromYX(
              notesEnvelope!.getMinY(), notesEnvelope!.getMaxX()));
      notesEnvHeight = CoordinateUtilities.getDistance(
          Coordinate.fromYX(notesEnvelope!.getMinY(), notesEnvelope!.getMinX()),
          Coordinate.fromYX(
              notesEnvelope!.getMaxY(), notesEnvelope!.getMinX()));
    }
      ...
      for (var log in logs) {
        ...
        for (int i = 0; i < pointsList.length - 1; i++) {
          ...
          if (ldp1.filtered_lat != null) {
            ...
            logsEnvelope!
                .expandToInclude(ldp2.filtered_lon!, ldp2.filtered_lat!);
          } else {
            distance = CoordinateUtilities.getDistance(
                ...
                if (logsCount > 0) ...[
                  ListTile(
                    title: SmashUI.titleText('Logs temporal envelope',
                        color: SmashColors.mainDecorations),
                    subtitle: SmashUI.normalText(
                        'From $logStartDate to $logEndDate.'),
                  ),
                  ListTile(
                    ...
                    child: Row(
                      ...
                      children: [
                        Expanded(
                          ...
                          child: TextField(
                            controller:
                                TextEditingController(text: logNameFilter),
