/*
 * Copyright (c) 2019. Antonello Andrea (www.hydrologis.com). All rights reserved.
 * Use of this source code is governed by a GPL3 license that can be
 * found in the LICENSE file.
 */
import 'package:sqflite/sqflite.dart';
import 'package:latlong/latlong.dart';
import 'package:geopaparazzi_light/eu/geopaparazzi/library/utils/utils.dart';

/////////////////////////////////////
/////////////////////////////////////
/// TABLE NAMES
/////////////////////////////////////
/////////////////////////////////////

String TABLE_NOTES = "notes";
String TABLE_GPSLOGS = "gpslogs";
String TABLE_GPSLOG_PROPERTIES = "gpslogsproperties";
String TABLE_GPSLOG_DATA = "gpslogsdata";

/////////////////////////////////////
/////////////////////////////////////
/// NOTES
/////////////////////////////////////
/////////////////////////////////////

/*
 * id of the note, Generated by the db.
 */
String NOTES_COLUMN_ID = "_id";
/*
 * Longitude of the note in WGS84.
 */
String NOTES_COLUMN_LON = "lon";
/*
 * Latitude of the note in WGS84.
 */
String NOTES_COLUMN_LAT = "lat";
/*
 * Elevation of the note.
 */
String NOTES_COLUMN_ALTIM = "altim";
/*
 * Timestamp of the note.
 */
String NOTES_COLUMN_TS = "ts";
/*
 * Description of the note.
 */
String NOTES_COLUMN_DESCRIPTION = "description";
/*
 * Simple text of the note.
 */
String NOTES_COLUMN_TEXT = "text";
/*
 * Form data of the note.
 */
String NOTES_COLUMN_FORM = "form";
/*
 * Is dirty field =0 = false, 1 = true)
 */
String NOTES_COLUMN_ISDIRTY = "isdirty";
/*
 * Style of the note.
 */
String NOTES_COLUMN_STYLE = "style";

class Note {
  int id;
  String text;
  String description;
  int timeStamp;
  double lon;
  double lat;
  double altim;
  String style;
  String form;
  int isDirty = 1;

  Map<String, dynamic> toMap() {
    var map = {
      NOTES_COLUMN_LAT: lat,
      NOTES_COLUMN_LON: lon,
      NOTES_COLUMN_TS: timeStamp,
      NOTES_COLUMN_TEXT: text,
      NOTES_COLUMN_ISDIRTY: isDirty,
    };
    if (id != null) {
      map[NOTES_COLUMN_ID] = id;
    }
    if (form != null) {
      map[NOTES_COLUMN_FORM] = form;
    }
    if (altim != null) {
      map[NOTES_COLUMN_ALTIM] = altim;
    }
    if (description != null) {
      map[NOTES_COLUMN_DESCRIPTION] = description;
    }
    if (style != null) {
      map[NOTES_COLUMN_STYLE] = style;
    }
    return map;
  }
}

/////////////////////////////////////
/////////////////////////////////////
/// LOGS
/////////////////////////////////////
/////////////////////////////////////

/*
 * id of the log, Generated by the db.
 */
String LOGS_COLUMN_ID = "_id";
/*
 * the start UTC timestamp.
 */
String LOGS_COLUMN_STARTTS = "startts";
/*
 * the end UTC timestamp.
 */
String LOGS_COLUMN_ENDTS = "endts";
/*
 * The length of the track in meters, as last updated.
 */
String LOGS_COLUMN_LENGTHM = "lengthm";
/*
 * Is dirty field =0=false, 1=true)
 */
String LOGS_COLUMN_ISDIRTY = "isdirty";
/*
 * the name of the log.
 */
String LOGS_COLUMN_TEXT = "text";

/*
 * id of the log, Generated by the db.
 */
String LOGSPROP_COLUMN_ID = "_id";
/*
 * field for log visibility.
 */
String LOGSPROP_COLUMN_VISIBLE = "visible";
/*
 * the lgo stroke width.
 */
String LOGSPROP_COLUMN_WIDTH = "width";
/*
 * the log stroke color.
 */
String LOGSPROP_COLUMN_COLOR = "color";
/*
 * the id of the parent gps log.
 */
String LOGSPROP_COLUMN_LOGID = "logid";

/*
 * id of the log point, Generated by the db.
 */
String LOGSDATA_COLUMN_ID = "_id";
/*
 * the longitude of the point.
 */
String LOGSDATA_COLUMN_LON = "lon";
/*
 * the latitude of the point.
 */
String LOGSDATA_COLUMN_LAT = "lat";
/*
 * the elevation of the point.
 */
String LOGSDATA_COLUMN_ALTIM = "altim";
/*
 * the UTC timestamp
 */
String LOGSDATA_COLUMN_TS = "ts";
/*
 * the id of the parent gps log.
 */
String LOGSDATA_COLUMN_LOGID = "logid";

class Log {
  int id;
  int startTime;
  int endTime;
  double lengthm;
  String text;
  int isDirty;

  List<LatLng> logData = [];

  Map<String, dynamic> toMap() {
    var map = {
      LOGS_COLUMN_STARTTS: startTime,
      LOGS_COLUMN_ENDTS: endTime,
      LOGS_COLUMN_LENGTHM: lengthm,
      LOGS_COLUMN_TEXT:
          text == null ? ISO8601_TS_FORMATTER.format(new DateTime.now()) : text,
      LOGS_COLUMN_ISDIRTY: isDirty,
    };
    if (id != null) {
      map[LOGS_COLUMN_ID] = id;
    }
    return map;
  }
}

class LogProperty {
  int id;
  int isVisible;
  double width;
  String color;
  int logid;

  Map<String, dynamic> toMap() {
    var map = {
      LOGSPROP_COLUMN_COLOR: color,
      LOGSPROP_COLUMN_VISIBLE: isVisible,
      LOGSPROP_COLUMN_WIDTH: width,
      LOGSPROP_COLUMN_LOGID: logid,
    };
    if (id != null) {
      map[LOGSPROP_COLUMN_ID] = id;
    }
    return map;
  }
}

class LogDataPoint {
  int id;
  double lon;
  double lat;
  double altim;
  int ts;
  int logid;

  Map<String, dynamic> toMap() {
    var map = {
      LOGSDATA_COLUMN_LAT: lat,
      LOGSDATA_COLUMN_LON: lon,
      LOGSDATA_COLUMN_ALTIM: altim,
      LOGSDATA_COLUMN_TS: ts,
      LOGSDATA_COLUMN_LOGID: logid,
    };
    if (id != null) {
      map[LOGSDATA_COLUMN_ID] = id;
    }
    return map;
  }
}
