...
- **Go to**: uses the device's geocoding service to look up an address and move the map to the result.
...
- **Available icons**: opens the icons view, from which you can select icons to be used as default markers in the notes properties view.
- **Offline maps**: SMASH has a built-in downloader for Mapsforge map files (see [Supported map types](map_types.md)) covering most of the world; tapping a region starts the download, which can take a while for large countries.
  ...
  :::
- **Andromaps link**: opens the OpenAndroMaps download page in the browser, for additional Mapsforge-compatible maps that also support isolines.
- **Form builder**: opens the form builder interface, allowing you to create and edit forms for data collection within the project.
...
In this view you can pick, from the full set, which ones should be offered as markers in the notes properties view.
